...
export default class SubscriptionsService {
  ...
  private async sync_youtube() {
    ...
    const subscriptions = await this.sync_youtube_subscriptions(token);
    await this.sync_youtube_videos(token, subscriptions);
  }

  private async sync_youtube_subscriptions(token: ServiceCredentials) {
    const database = useDatabase();
    ...
    const subscriptions = await this.get_youtube_subscriptions(token.access_token);
  ...
  private async sync_youtube_videos(
    ...
    for (const subscription of subscriptions.upsert) {
      const videos = await this.get_youtube_videos_by_channel(
        token.access_token,
